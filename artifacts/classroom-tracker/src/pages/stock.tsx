...
export default function Stock() {
  ...
  const maxQty = supplies ? Math.max(...supplies.map((s) => s.quantity), 1) : 1;

  const inStock = supplies?.filter((s) => s.quantity > s.reorderThreshold).length ?? 0;
  const lowStock = supplies?.filter((s) => s.quantity <= s.reorderThreshold && s.quantity > 0).length ?? 0;
  const outOfStock = supplies?.filter((s) => s.quantity === 0).length ?? 0;
                  ...
                  <CardContent className="p-5">
                    <div className="flex items-start justify-between mb-3">
                      ...
                      {isOut ? (
                        ...
                        <Badge className="bg-orange-100 text-orange-800 hover:bg-orange-100 border-orange-200 text-xs shrink-0">
                          Low Stock
                        </Badge>
                      ) : (
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <span
                          className={`text-base font-bold tabular-nums ${
                            isOut ? "text-destructive" : isLow ? "text-orange-600" : "text-foreground"
                          }`}
                        >
                          {supply.quantity}
                        </span>
