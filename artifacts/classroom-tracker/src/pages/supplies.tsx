...
function CreateSupplyDialog({ categories, onOpenChange, open }: { categories: string[], open: boolean, onOpenChange: (open: boolean) => void }) {
  ...
  const onSubmit = (values: z.infer<typeof supplySchema>) => {
...
export default function Supplies() {
  ...
  const { data: supplies, isLoading } = useListSupplies(
    { category, search },
    { query: { queryKey: getListSuppliesQueryKey({ category, search }) } }
  );
      ...
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <p className="text-muted-foreground mt-1">Manage all classroom supplies and monitor stock levels.</p>
          </div>
          <CreateSupplyDialog categories={categories} open={createOpen} onOpenChange={setCreateOpen} />
        </div>
                      ...
                      <TableRow key={supply.id}>
                        ...
                        <TableCell>
                          <div className="flex items-center gap-2">
                            ...
                            {isOutOfStock ? (
                              <Badge variant="destructive" className="ml-2 py-0">Out of Stock</Badge>
                            ) : isLowStock ? (
                              <Badge className="bg-orange-100 text-orange-800 hover:bg-orange-100 border-orange-200 ml-2 py-0">
                                Low Stock
                              </Badge>
                            ) : null}
                          </div>
