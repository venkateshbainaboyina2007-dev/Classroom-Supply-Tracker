...
export default function SupplyDetail({ params }: { params: { id: string } }) {
  ...
  const { data: usageLogs, isLoading: logsLoading } = useListUsageLogs(
    { supplyId: id },
    { query: { queryKey: getListUsageLogsQueryKey({ supplyId: id }), enabled: !isNaN(id) } }
  );
  ...
  const editForm = useForm<z.infer<typeof editSupplySchema>>({
  ...
  const onEditSubmit = (values: z.infer<typeof editSupplySchema>) => {
