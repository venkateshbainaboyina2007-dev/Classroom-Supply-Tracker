...
function RestockDialog({
  ...
  const onSubmit = (values: z.infer<typeof restockSchema>) => {
    restockSupply.mutate(
      { id: supplyId, data: values },
      {
        onSuccess: () => {
...
export default function OutOfStock() {
  ...
  const { data: allSupplies, isLoading } = useListSupplies(
    {},
    { query: { queryKey: getListSuppliesQueryKey() } }
  );

  const supplies = allSupplies?.filter((s) => s.quantity === 0) ?? [];
      ...
      <div className="space-y-6">
        <div className="flex items-start justify-between">
          ...
          {!isLoading && supplies.length > 0 && (
            <Badge variant="destructive" className="text-base px-3 py-1 mt-1">
            ...
            <Table>
              ...
              <TableBody>
                {isLoading ? (
                  ...
                  supplies.map((supply) => (
                    <TableRow key={supply.id} className="bg-destructive/5 hover:bg-destructive/10">
                      ...
                      <TableCell className="text-right">
                        <RestockDialog
                          ...
                          defaultQuantity={supply.reorderQuantity}
                          open={!!activeDialogs[supply.id]}
                          onOpenChange={(open) => handleDialogChange(supply.id, open)}
                        />
                      </TableCell>
                    </TableRow>
                  ))
